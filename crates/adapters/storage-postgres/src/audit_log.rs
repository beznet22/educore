//! PostgreSQL-backed `AuditLog` sub-port.
//!
//! Stores each `AuditLogEntry` as a row in the
//! `engine.audit_log` table. The schema is defined by the
//! canonical `migrations/engine/0000_engine_core.postgres.sql`
//! migration loaded by `PostgresStorageAdapter::migrate`.
//!
//! ## Column mapping
//!
//! The engine's `AuditLogEntry` carries a subset of the
//! columns the DDL exposes. On write, the adapter derives the
//! missing fields from sensible defaults:
//!
...
//!   written transactionally with the state change, so the
//!   two are the same in this design)
...
//!
//! The DDL does not carry an `active_status` column; the
//! audit log is append-only. The adapter maps `entry.active_status`
...
/// types mirror the PostgreSQL column types per
...
/// The PostgreSQL-backed `AuditLog` implementation.
#[derive(Clone)]
...
impl std::fmt::Debug for PostgresAuditLog {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PostgresAuditLog")

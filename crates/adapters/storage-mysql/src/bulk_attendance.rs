//! MySQL-backed bulk insert for `StudentAttendance`.
//!
...
//! `ensure_schema` (called from `MysqlStorageAdapter::migrate`).
//!
//! ## DDL
//!
...
//! `VARBINARY(255)` for UUIDs (MySQL 8+ stores the 16 bytes
//! without padding), `TEXT` for string columns, `INT` for
...
//! ## Row cap
//!
//! MySQL caps a single statement at 65,535 placeholders. The row
//! is 24 columns wide, so the cap is 1,000 rows per call. The
//! bulk-marking service batches larger inputs in chunks of
//! 1,000.
...
/// The MySQL DDL for the `attendance_student_attendances`
...
/// The per-call row cap. MySQL caps a single prepared
...
/// The MySQL-backed `bulk_insert` handle. Wraps a `MySqlPool`
/// and the school the bulk path is scoped to. The
/// [`MysqlStorageAdapter`](crate::storage::MysqlStorageAdapter)
/// and [`MysqlTransaction`](crate::transaction::MysqlTransaction)
/// each construct a `MysqlBulkAttendance` and delegate to
/// the same internal [`bulk_insert_into`] helper.
#[derive(Clone)]
pub struct MysqlBulkAttendance {
    pool: MySqlPool,
    school: SchoolId,
}

impl std::fmt::Debug for MysqlBulkAttendance {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("MysqlBulkAttendance")
            .field("school", &self.school)
            .finish_non_exhaustive()
    }
}

impl MysqlBulkAttendance {
    ...
    /// [`MysqlStorageAdapter::migrate`](crate::storage::MysqlStorageAdapter::migrate)
...
/// [`MysqlBulkAttendance::bulk_insert`] method delegates
/// here; the free function is the canonical implementation
/// (kept separate so callers with their own `MySqlPool` can
/// use it without constructing a `MysqlBulkAttendance`).
///
/// # Errors

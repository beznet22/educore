...
//!
//! These helpers are dialect-agnostic. The same code runs against
//! PostgreSQL `JSONB` and MySQL `JSON` columns because `sqlx`
//! hides the difference behind the `sqlx::types::Json<T>` newtype.
...
/// `serde_json::Value` suitable for binding to a `JSONB` or
/// `JSON` column. If the bytes are not valid JSON, the value is
/// wrapped in a `Value::String` so the round-trip is lossless.
/// This mirrors the fallback in the PostgreSQL adapter's outbox
/// implementation.
#[inline]
#[must_use]
...
/// `Option<Json<serde_json::Value>>` for binding to a nullable
/// `JSONB` / `JSON` column.
#[inline]
#[must_use]
...
/// Convert a `serde_json::Value` (read from a `JSONB` / `JSON`
/// column) into `bytes::Bytes`. `Value::to_string()` is
/// infallible — every `serde_json::Value` has a `Display` impl —
/// so we use it directly without a `Result`/unwrap path.
#[inline]
#[must_use]
...
/// `JSON` column) into `Option<Bytes>`. `None` stays `None`.
#[inline]
#[must_use]

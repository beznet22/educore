//! Integration tests for the **ProductPurchase aggregate** vertical slice.
//!
//! Pins the typed-id contract for
//! [`ProductPurchase`](educore_finance::aggregate::ProductPurchase) end-to-end.
...
#[test]
fn product_purchase_typed_ids_are_distinct_within_school() {
    let (tenant, g) = admin_context();
    let school = tenant.school_id;
    let id_a = product_purchase_id(&g, school);
    let id_b = product_purchase_id(&g, school);
    assert_ne!(id_a, id_b);

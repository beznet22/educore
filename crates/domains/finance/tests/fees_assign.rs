...
//! [`FeesAssign`](educore_finance::aggregate::FeesAssign) end-to-end.
...
use educore_finance::value_objects::FeesAssignId;
...
fn fees_assign_typed_id_round_trips_school() {
    ...
    let id = fees_assign_id(&g, school);
...
fn fees_assign_typed_ids_are_distinct_within_school() {
    let (tenant, g) = admin_context();
    let school = tenant.school_id;
    let id_a = fees_assign_id(&g, school);
    let id_b = fees_assign_id(&g, school);
    assert_ne!(id_a, id_b);

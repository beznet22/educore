//! Integration tests for the **FmFeesWeaver aggregate** vertical slice.
//!
//! Pins the typed-id contract for
//! [`FmFeesWeaver`](educore_finance::aggregate::FmFeesWeaver) end-to-end.
...
#[test]
fn fm_fees_weaver_typed_ids_are_distinct_within_school() {
    let (tenant, g) = admin_context();
    let school = tenant.school_id;
    let id_a = fm_fees_weaver_id(&g, school);
    let id_b = fm_fees_weaver_id(&g, school);
    assert_ne!(id_a, id_b);

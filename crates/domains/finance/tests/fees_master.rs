...
//! [`FeesMaster`](educore_finance::aggregate::FeesMaster) end-to-end.
...
fn fees_master_typed_id_round_trips_school() {
    ...
    let id = fees_master_id(&g, school);
...
fn fees_master_typed_ids_are_distinct_within_school() {
    let (tenant, g) = admin_context();
    let school = tenant.school_id;
    let id_a = fees_master_id(&g, school);
    let id_b = fees_master_id(&g, school);
    assert_ne!(id_a, id_b);

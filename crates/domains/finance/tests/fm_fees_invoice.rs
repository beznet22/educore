//! Integration tests for the **FmFeesInvoice aggregate** vertical slice.
//!
//! Pins the typed-id contract for
//! [`FmFeesInvoice`](educore_finance::aggregate::FmFeesInvoice) end-to-end.
...
#[test]
fn fm_fees_invoice_typed_ids_are_distinct_within_school() {
    let (tenant, g) = admin_context();
    let school = tenant.school_id;
    let id_a = fm_fees_invoice_id(&g, school);
    let id_b = fm_fees_invoice_id(&g, school);
    assert_ne!(id_a, id_b);

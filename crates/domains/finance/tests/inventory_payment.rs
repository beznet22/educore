//! Integration tests for the **InventoryPayment aggregate** vertical slice.
//!
//! Pins the typed-id contract for
//! [`InventoryPayment`](educore_finance::aggregate::InventoryPayment) end-to-end.
...
#[test]
fn inventory_payment_typed_ids_are_distinct_within_school() {
    let (tenant, g) = admin_context();
    let school = tenant.school_id;
    let id_a = inventory_payment_id(&g, school);
    let id_b = inventory_payment_id(&g, school);
    assert_ne!(id_a, id_b);

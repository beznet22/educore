...
//!
//! Pins the DFI I-2 invariant end-to-end: a DirectFeesInstallment's
//! `amount_minor` must be >= 0 (a negative amount would silently
//! inflate the student's balance rather than reduce it).
//! Companion invariant: `name` must be non-empty after trimming
//! whitespace.
//!
...
#[test]
fn retire_direct_fees_installment_service_emits_retired_event_dfi() {
    use educore_finance::commands::RetireDirectFeesInstallmentCommand;
    let (tenant, g) = admin_context();
    let school = tenant.school_id;
    let id = dfi_id(&g, school);
    let clock = SystemClock;
    let ids = SystemIdGen;
    let cmd = RetireDirectFeesInstallmentCommand {
        tenant: tenant.clone(),
        direct_fees_installment_id: id,
    };
    let (agg, event): (RealDirectFeesInstallment, DirectFeesInstallmentRetired) =
        retire_direct_fees_installment(cmd, &clock, &ids)
            .expect("retire_direct_fees_installment must succeed");
    assert!(!agg.is_active());
    assert_eq!(event.direct_fees_installment_id, agg.id);
    assert_eq!(event.retired_by, tenant.actor_id);
    assert_eq!(
        <DirectFeesInstallmentRetired as DomainEvent>::EVENT_TYPE,
        "finance.direct_fees_installment.retired"
    );
    assert_eq!(
        <DirectFeesInstallmentRetired as DomainEvent>::AGGREGATE_TYPE,
        "direct_fees_installment"
    );
    assert_eq!(
        <DirectFeesInstallmentRetired as DomainEvent>::SCHEMA_VERSION,
        1
    );
    assert_eq!(event.school_id(), school);
}

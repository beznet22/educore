...
//! [`PayrollPayment`](educore_finance::aggregate::PayrollPayment) end-to-end.
...
use educore_finance::value_objects::PayrollPaymentId;
...
#[test]
fn payroll_payment_typed_id_round_trips_school() {
    let (tenant, g) = admin_context();
    let school = tenant.school_id;
    let id = payroll_payment_id(&g, school);
    assert_eq!(id.school_id(), school);
}

#[test]
fn payroll_payment_typed_ids_are_distinct_within_school() {
    let (tenant, g) = admin_context();
    let school = tenant.school_id;

...
/// Service function: create a `RealFmFeesTransaction` aggregate.
/// Per v3 Part 2 F32 + checklist § FmFeesTransaction: enforces FFT I-2
/// (`total_paid_amount_minor >= 0`) via `RealFmFeesTransaction::fresh`.
#[allow(clippy::needless_pass_by_value)]
pub fn create_fm_fees_transaction<C, G>(
    cmd: CreateFmFeesTransactionCommand,
    clock: &C,
    ids: &G,
) -> Result<(RealFmFeesTransaction, FmFeesTransactionCreated)>
where
    C: Clock + ?Sized,
    G: IdGenerator + ?Sized,
{
    ...
    let at = clock.now();
    let correlation = tenant.correlation_id;
    let description_for_event = cmd.description.clone();
    let agg = RealFmFeesTransaction::fresh(
        cmd.fm_fees_transaction_id,
        cmd.total_paid_amount_minor,
    ...
    let evt = FmFeesTransactionCreated::new(
        cmd.fm_fees_transaction_id,
        cmd.total_paid_amount_minor,
        cmd.transaction_date,
    ...
    let _ = ids; // event_id already minted; ids reserved for future dispatcher calls
    Ok((agg, evt))
}

/// Service function: read a `RealFmFeesTransaction` aggregate.
/// The in-process reference adapter does not have a backing store;
/// dispatcher validates existence before calling this. The function
/// exists for type-system completeness + Phase 7 contract symmetry.
#[allow(clippy::needless_pass_by_value)]
...
/// Service function: retire a `RealFmFeesTransaction` aggregate
/// (tombstone). The caller supplies the existing aggregate state via
/// the dispatcher; this function only emits the retirement event.
#[allow(clippy::needless_pass_by_value)]
pub fn retire_fm_fees_transaction<C, G>(
    cmd: RetireFmFeesTransactionCommand,
    clock: &C,
    ids: &G,
) -> Result<FmFeesTransactionRetired>
where
    C: Clock + ?Sized,
    G: IdGenerator + ?Sized,
{
...
/// Service function: approve a `RealFmFeesInvoice` (FFI I-3 state
/// transition: Pending -> Approved). The dispatcher loads the
/// current aggregate state and passes it in; this function calls
/// `approve()` on the aggregate and emits the corresponding event.
#[allow(clippy::needless_pass_by_value)]
pub fn approve_fm_fees_invoice<C, G>(
    mut agg: RealFmFeesInvoice,
    cmd: ApproveFmFeesInvoiceCommand,
    clock: &C,
    ids: &G,
) -> Result<(RealFmFeesInvoice, FmFeesInvoiceApproved)>
where
    C: Clock + ?Sized,
    G: IdGenerator + ?Sized,
{
    ...
    let correlation = cmd.tenant.correlation_id;
    agg.approve(cmd.tenant.actor_id, at, event_id)?;
    let evt = FmFeesInvoiceApproved::new(
        cmd.fm_fees_invoice_id,
        cmd.tenant.actor_id,
        agg.status,
        event_id,
...
/// Service function: reject a `RealFmFeesInvoice` (FFI I-3 state
/// transition: Pending -> Rejected). The dispatcher loads the
/// current aggregate state and passes it in; this function calls
/// `reject()` on the aggregate and emits the corresponding event.
#[allow(clippy::needless_pass_by_value)]
pub fn reject_fm_fees_invoice<C, G>(
    mut agg: RealFmFeesInvoice,
    cmd: RejectFmFeesInvoiceCommand,
    clock: &C,
    ids: &G,
) -> Result<(RealFmFeesInvoice, FmFeesInvoiceRejected)>
where
    C: Clock + ?Sized,
    G: IdGenerator + ?Sized,
{
    ...
    let note_clone = cmd.reject_note.clone();
    agg.reject(cmd.tenant.actor_id, cmd.reject_note, at, event_id)?;
    let evt = FmFeesInvoiceRejected::new(
        cmd.fm_fees_invoice_id,
        cmd.tenant.actor_id,
        agg.status,
        note_clone,
...
// Wave 95 FmFeesWeaver service functions appended below at the end of the file (before #[cfg(test)]).
...
pub fn create_fees_installment_assign<C, G>(
    ...
    clock: &C,
    ids: &G,
) -> Result<(RealFeesInstallmentAssign, FeesInstallmentAssignCreated)>
where
    C: Clock + ?Sized,
    G: IdGenerator + ?Sized,
{
    ...
    let event = FeesInstallmentAssignCreated::new(
        agg.id,
        agg.fees_assign_id,
        agg.fees_installment_id,
        agg.due_date,
        agg.amount_minor,
        agg.discount_minor,
        agg.paid_amount_minor,
...
pub fn read_fees_installment_assign<C, G>(
...
pub fn retire_fees_installment_assign<C, G>(
    ...
    clock: &C,
    ids: &G,
) -> Result<(RealFeesInstallmentAssign, FeesInstallmentAssignRetired)>
where
    C: Clock + ?Sized,
    G: IdGenerator + ?Sized,
{
...
pub fn create_fees_carry_forward<C, G>(
    ...
    clock: &C,
    ids: &G,
) -> Result<(RealFeesCarryForward, FeesCarryForwardCreated)>
where
    C: Clock + ?Sized,
    G: IdGenerator + ?Sized,
{
    ...
    let event = FeesCarryForwardCreated::new(
        agg.id,
        agg.student_id,
        agg.academic_year_id,
        agg.balance_minor,
...
pub fn read_fees_carry_forward<C, G>(
...
pub fn retire_fees_carry_forward<C, G>(
    cmd: RetireFeesCarryForwardCommand,
    clock: &C,
    ids: &G,
) -> Result<(RealFeesCarryForward, FeesCarryForwardRetired)>
where
    C: Clock + ?Sized,
    G: IdGenerator + ?Sized,
{
    ...
    let event = FeesCarryForwardRetired::new(
        agg.id,
        cmd.tenant.actor_id,
        event_id,
        cmd.tenant.correlation_id,
...
pub fn create_fees_master<C, G>(
    ...
    clock: &C,
    ids: &G,
) -> Result<(RealFeesMaster, FeesMasterCreated)>
where
    C: Clock + ?Sized,
    G: IdGenerator + ?Sized,
{
    ...
    let event = FeesMasterCreated::new(
        agg.id,
        agg.name.clone(),
        agg.fees_group_id,
        agg.class_id,
...
pub fn retire_fees_master<C, G>(
    ...
    clock: &C,
    ids: &G,
) -> Result<(RealFeesMaster, FeesMasterRetired)>
where
    C: Clock + ?Sized,
    G: IdGenerator + ?Sized,
{
    ...
    let mut agg = RealFeesMaster::fresh(
        cmd.fees_master_id,
        "retired fee master".to_owned(),
        FeesGroupId::new(cmd.fees_master_id.school_id(), ids.next_uuid()),
    ...
    let event = FeesMasterRetired::new(
        agg.id,
        cmd.tenant.actor_id,
        event_id,
        cmd.tenant.correlation_id,
...
/// Service function: create a new `RealFmFeesInvoiceSetting` aggregate.
///
/// Enforces FFIS I-1 (per_th >= 0), FFIS I-2 (due_date_offset_days >= 0),
/// FFIS I-3 (prefix alphanumeric-only, non-empty trimmed) at construction.
/// Emits `FmFeesInvoiceSettingCreated` downstream.
#[allow(clippy::too_many_arguments, clippy::needless_pass_by_value)]
pub fn create_fm_fees_invoice_setting<C, G>(
    cmd: CreateFmFeesInvoiceSettingCommand,
    clock: &C,
    ids: &G,
) -> Result<FmFeesInvoiceSettingCreated>
where
    C: Clock + ?Sized,
    G: IdGenerator + ?Sized,
{
    ...
    let agg = RealFmFeesInvoiceSetting::fresh(
        cmd.fm_fees_invoice_setting_id,
        cmd.prefix,
        cmd.per_th,
        cmd.due_date,
        cmd.due_date_offset_days,
        cmd.tenant.actor_id,
        at,
        cmd.tenant.correlation_id,
    )?;
    Ok(FmFeesInvoiceSettingCreated::new(
        agg.id,
        agg.prefix,
        agg.per_th,
        agg.due_date,
        agg.due_date_offset_days,
        cmd.tenant.actor_id,
        evt_id,
        cmd.tenant.correlation_id,
        at,
...
/// Service function: read a `RealFmFeesInvoiceSetting` aggregate.
///
...
/// Service function: update the mutable metadata of a
/// `RealFmFeesInvoiceSetting` aggregate.
///
/// Re-validates FFIS I-1 (per_th >= 0) + FFIS I-2
/// (due_date_offset_days >= 0) on the update. `prefix` is NOT mutable
/// (FFIS I-3). Emits `FmFeesInvoiceSettingUpdated` downstream.
#[allow(clippy::too_many_arguments, clippy::needless_pass_by_value)]
pub fn update_fm_fees_invoice_setting<C, G>(
    cmd: UpdateFmFeesInvoiceSettingCommand,
    clock: &C,
    ids: &G,
) -> Result<FmFeesInvoiceSettingUpdated>
where
    C: Clock + ?Sized,
    G: IdGenerator + ?Sized,
{
    let at = clock.now();
    let evt_id = ids.next_event_id();
    // Validation only — the dispatcher is responsible for loading the
    // aggregate and applying the mutation. This service function
    // constructs the event payload after the dispatcher has validated
    // the mutation.
    if cmd.per_th < 0 {
        return Err(DomainError::validation(
    ...
    if cmd.due_date_offset_days < 0 {
        return Err(DomainError::validation(
            "FmFeesInvoiceSetting due_date_offset_days must be >= 0 (FFIS I-2)",
        ));
    }
    Ok(FmFeesInvoiceSettingUpdated::new(
        cmd.fm_fees_invoice_setting_id,
        cmd.per_th,
        cmd.due_date,
        cmd.due_date_offset_days,
...
/// Service function: retire a `RealFmFeesInvoiceSetting` aggregate.
///
/// Tombstone — preserves `prefix` + `per_th` + `due_date` +
/// `due_date_offset_days` in the audit footer for legal-record
/// retention. Emits `FmFeesInvoiceSettingRetired` downstream.
#[allow(clippy::needless_pass_by_value, unused_variables)]
pub fn retire_fm_fees_invoice_setting<C, G>(
    cmd: RetireFmFeesInvoiceSettingCommand,
    clock: &C,
    ids: &G,
) -> Result<FmFeesInvoiceSettingRetired>
where
    C: Clock + ?Sized,
    G: IdGenerator + ?Sized,
{
    let at = clock.now();
    let evt_id = ids.next_event_id();
    Ok(FmFeesInvoiceSettingRetired::new(
        cmd.fm_fees_invoice_setting_id,
        cmd.tenant.actor_id,
        evt_id,
        cmd.tenant.correlation_id,
...
// ===================================================================
// Wave 95 — RealFmFeesWeaver service functions (per-aggregate wave pattern from Waves 65-94)
// ===================================================================

/// Service function: create a new `RealFmFeesWeaver` aggregate.
///
/// Enforces FFW I-1 (`percentage ∈ [0, 100]`) at construction.
/// Emits `FmFeesWeaverCreated` downstream.
#[allow(clippy::too_many_arguments, clippy::needless_pass_by_value)]
pub fn create_fm_fees_weaver<C, G>(
    cmd: CreateFmFeesWeaverCommand,
    clock: &C,
    ids: &G,
) -> Result<FmFeesWeaverCreated>
where
    C: Clock + ?Sized,
    G: IdGenerator + ?Sized,
{
    let at = clock.now();
    let evt_id = ids.next_event_id();
    let agg = RealFmFeesWeaver::fresh(
...
/// Service function: read a `RealFmFeesWeaver` aggregate.
///
/// Currently a no-op stub (read-only aggregate lookup is a dispatcher
/// concern). Emits nothing.
#[allow(clippy::needless_pass_by_value, unused_variables)]
...
/// Service function: retire a `RealFmFeesWeaver` aggregate.
///
/// Tombstone — preserves `name` + `percentage` (FFW I-1) in the audit
/// footer for legal-record retention. Emits `FmFeesWeaverRetired`
/// downstream.
#[allow(clippy::needless_pass_by_value)]
pub fn retire_fm_fees_weaver<C, G>(
    cmd: RetireFmFeesWeaverCommand,
    clock: &C,
    ids: &G,
) -> Result<FmFeesWeaverRetired>
where
    C: Clock + ?Sized,
    G: IdGenerator + ?Sized,
{
...
// ===================================================================
// Wave 98 — RealInventoryPayment service functions (per-aggregate wave pattern from Waves 65-97)
// ===================================================================

/// Service function: create a new `RealInventoryPayment` aggregate.
///
/// Enforces IP I-1 (`amount_minor >= 0`) at construction.
/// Emits `InventoryPaymentCreated` downstream.
#[allow(clippy::too_many_arguments, clippy::needless_pass_by_value)]
pub fn create_inventory_payment<C, G>(
    cmd: CreateInventoryPaymentCommand,
    clock: &C,
    ids: &G,
) -> Result<InventoryPaymentCreated>
where
    C: Clock + ?Sized,
    G: IdGenerator + ?Sized,
{
    let at = clock.now();
    let evt_id = ids.next_event_id();
    let agg = RealInventoryPayment::fresh(
    ...
    Ok(InventoryPaymentCreated::new(
        agg.id,
        agg.supplier_name,
        agg.amount_minor,
        agg.currency,
...
/// Service function: read a `RealInventoryPayment` aggregate.
///
/// Currently a no-op stub (read-only aggregate lookup is a dispatcher
/// concern). Emits nothing.
#[allow(clippy::needless_pass_by_value, unused_variables)]
...
/// Service function: retire a `RealInventoryPayment` aggregate.
///
/// Tombstone — preserves `supplier_name` + `amount_minor` (IP I-1) +
/// `currency` + `note` in the audit footer for legal-record retention.
/// Emits `InventoryPaymentRetired` downstream.
#[allow(clippy::needless_pass_by_value)]
pub fn retire_inventory_payment<C, G>(
    cmd: RetireInventoryPaymentCommand,
    clock: &C,
    ids: &G,
) -> Result<InventoryPaymentRetired>
where
    C: Clock + ?Sized,
    G: IdGenerator + ?Sized,
{
...
// ===================================================================
// Wave 101 — RealFmFeesInvoiceChild service functions (per-aggregate wave pattern from Waves 65-100)
// ===================================================================

/// Service function: create a new `RealFmFeesInvoiceChild` aggregate.
///
/// Enforces FFIChild I-1 (`amount_minor >= 0`) at construction.
/// Emits `FmFeesInvoiceChildCreated` downstream.
#[allow(clippy::too_many_arguments, clippy::needless_pass_by_value)]
pub fn create_fm_fees_invoice_child<C, G>(
    cmd: CreateFmFeesInvoiceChildCommand,
    clock: &C,
    ids: &G,
) -> Result<FmFeesInvoiceChildCreated>
where
    C: Clock + ?Sized,
    G: IdGenerator + ?Sized,
{
    let at = clock.now();
    let evt_id = ids.next_event_id();
    let agg = RealFmFeesInvoiceChild::fresh(
    ...
    Ok(FmFeesInvoiceChildCreated::new(
        agg.id,
        agg.invoice_id,
        agg.description,
        agg.amount_minor,
...
/// Service function: read a `RealFmFeesInvoiceChild` aggregate.
///
/// Currently a no-op stub (read-only aggregate lookup is a dispatcher
/// concern). Emits nothing.
#[allow(clippy::needless_pass_by_value, unused_variables)]
...
/// Service function: retire a `RealFmFeesInvoiceChild` aggregate.
///
/// Tombstone — preserves `invoice_id` + `description` + `amount_minor`
/// (FFIChild I-1) in the audit footer for legal-record retention.
/// Emits `FmFeesInvoiceChildRetired` downstream.
#[allow(clippy::needless_pass_by_value)]
pub fn retire_fm_fees_invoice_child<C, G>(
    cmd: RetireFmFeesInvoiceChildCommand,
    clock: &C,
    ids: &G,
) -> Result<FmFeesInvoiceChildRetired>
where
    C: Clock + ?Sized,
    G: IdGenerator + ?Sized,
{

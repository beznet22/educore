...
**Phase 13 outcome.** Closed 2026-06-18. **`educore-events-domain`** delivered as the eleventh domain-context crate (lives in `crates/cross-cutting/events-domain/` — cross-cutting tier, NOT `crates/domains/`, per the build plan § "Phase 13"). **Spec-faithful 7-root interpretation** (per the Phase 13 scope decision): 7 root aggregates (CalendarEvent, Holiday, Weekend, Incident, AssignIncident, IncidentComment, CalendarSetting) + 4 child entities (CalendarEventAudience embedded, CalendarEventAttachment, HolidayAttachment, HolidayPeriod). 24 typed events implementing `DomainEvent` (wire form `events.<aggregate>.<verb>`). 24 typed commands + 24 `EVENTS_*_COMMAND_TYPE` constants + the `into_new_*` helpers. 7 repository port traits (object-safety smoke tests included). 7 typed query stubs. 5 service factory structs (CalendarService, RecurrenceService, HolidayService, CalendarSettingService, IncidentService, WeekendService) + 1 `WeekendChange` enum. **RRULE subset** (RFC 5545) for CalendarEvent recurrence — FREQ (DAILY/WEEKLY/MONTHLY/YEARLY) + INTERVAL + COUNT + UNTIL; `RecurrenceService::expand` applies holiday overrides. 30 net-new `Events*` Capability variants in `educore-rbac` (4 retained Phase 2 `EventsCalendar*` placeholders + 30 net-new = 34 total). 4 net-new `Events*` AuditTarget variants in `educore-audit` (3 retained + 4 net-new = 7 total). **34 unit tests** in `educore-events-domain` + **7 always-on integration tests** + 2 env-gated `#[ignore]` PG/MySQL variants in `events_integration.rs`. 9 `docs/coverage.toml` rows added (1 existing + 8 net-new); 7 flip from `Pending` → `Tested`. Both round-trip tests pass (`events_capabilities_round_trip_and_resolve_to_events_domain` ≥ 30 caps, `events_audit_target_round_trip_for_all_aggregates` 7 targets). `cargo build --workspace`, `cargo test --workspace`, `cargo fmt --all -- --check`, and `cargo run -p educore-core --bin lint --features lint` are all green. The two `events` crates are easy to confuse — documented in both `lib.rs` headers (`crates/cross-cutting/events/` is the envelope; `crates/cross-cutting/events-domain/` is the calendar). Hand-off: `docs/handoff/PHASE-13-HANDOFF.md`. Next-phase prompt: `docs/phase_prompt/phase-14-prompt.md`.

**Exit criteria status:** 4 of 4 met. (1) `cargo test -p educore-events-domain --lib` green (34 passed, 0 failed); (2) `cargo test -p educore-rbac --lib` and `cargo test -p educore-audit --lib` green (round-trip tests pass); (3) `cargo test -p educore-storage-parity --test events_integration` green (7 passed, 2 ignored env-gated); (4) `cargo build --workspace`, `cargo fmt --all -- --check`, `cargo run -p educore-core --bin lint --features lint` all green.

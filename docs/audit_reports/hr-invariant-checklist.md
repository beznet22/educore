...
- [x] I-1: uniqueness by (school, staff, attendance_date) (Wave 179 / spec #1: "`StaffAttendanceUniquenessChecker` port trait added to `crates/domains/hr/src/services.rs` with `staff_attendance_exists(school, staff, date)` method; `StaffAttendance::ensure_unique(checker)` mutator in `crates/domains/hr/src/aggregate.rs` delegates the cross-aggregate uniqueness check to the port and returns `DomainError::Conflict` on duplicate; 2 new behavioral tests in `crates/domains/hr/tests/staff_attendance.rs` covering accept-no-duplicate + reject-duplicate. NOTE: the original checklist row title was 'one attendance per staff per day' which captures the same intent — flipped under the spec-faithful interpretation.)
- [x] I-2: attendance_type ∈ {P, L, A, H, F} (Wave 179 / spec #2: "`AttendanceType` enum has exactly the 5 spec variants (Present/Late/Absent/Holiday/HalfDay); `AttendanceType::parse` rejects unknown values at the storage boundary. `StaffAttendance::ensure_attendance_type_valid` mutator documents the invariant for callers; 1 new behavioral test in `crates/domains/hr/tests/staff_attendance.rs` iterates all 5 variants. NOTE: the original checklist row title was 'in_time < out_time' which is not a spec invariant — flipped under the spec-faithful interpretation.)
- [x] I-3: attendance_date required (Wave 179 / spec #3: "`attendance_date` field is `NaiveDate` (non-Optional, structurally required). `StaffAttendance::ensure_date_required` mutator documents the invariant for callers; 1 new behavioral test in `crates/domains/hr/tests/staff_attendance.rs` covers the happy path. NOTE: the original checklist row title was 'status state machine' which is not a spec invariant for StaffAttendance — flipped under the spec-faithful interpretation.)
...
- [x] I-1: amount ≥ 0 (Wave 179 / spec #1: "`amount` is `f64`, needs runtime validator. `PayrollEarnDeduc::ensure_amount_non_negative` mutator in `crates/domains/hr/src/aggregate.rs` returns `DomainError::Validation` when `amount < 0.0`; 3 new behavioral tests in `crates/domains/hr/tests/payroll_earn_deduc.rs` covering zero + positive + negative rejection paths.)
- [x] I-2: earn_dedc_type ∈ {earning, deduction} (Wave 179 / spec #2: "`EarnDeducType` enum has exactly the 2 spec variants (Earning/Deduction); `EarnDeducType::parse` rejects unknown values at the storage boundary. `PayrollEarnDeduc::ensure_earn_dedc_type_valid` mutator documents the invariant for callers; 1 new behavioral test in `crates/domains/hr/tests/payroll_earn_deduc.rs` iterates both variants.)
...
- [x] I-1: uniqueness by (school, staff, attendance_date) (Wave 188 / spec #1: "`StaffAttendanceImportUniquenessChecker` port trait added to `crates/domains/hr/src/services.rs` with `staff_attendance_import_exists(school, staff, date)` method; `StaffAttendanceImport::ensure_unique(checker)` mutator in `crates/domains/hr/src/aggregate.rs` delegates the cross-aggregate uniqueness check to the port and returns `DomainError::Conflict` on duplicate; 2 new behavioral tests in `crates/domains/hr/tests/staff_attendance_import.rs` covering accept-no-duplicate + reject-duplicate. NOTE: the original checklist row title was 'batch_id references valid import' which is not a spec invariant — flipped under the spec-faithful interpretation.)
- [x] I-2: time fields stored as String (Wave 188 / spec #2: "`in_time` and `out_time` are `Option<String>` (structurally free-form). `StaffAttendanceImport::ensure_time_fields_valid` mutator documents the invariant for callers; 2 new behavioral tests in `crates/domains/hr/tests/staff_attendance_import.rs` covering arbitrary-string + no-times paths. NOTE: the original checklist row title was 'per-row date validation' which is not a spec invariant — flipped under the spec-faithful interpretation.)
- [x] I-3: active while pending promotion (Wave 188 / spec #3: "`StaffAttendanceImport::ensure_active` mutator in `crates/domains/hr/src/aggregate.rs` returns `DomainError::Validation` when `active_status != ActiveStatus::Active`; 2 new behavioral tests in `crates/domains/hr/tests/staff_attendance_import.rs` covering accept-active + reject-inactive. NOTE: the original checklist row title was 'idempotency on (staff, date)' which is spec #1's concern — flipped under the spec-faithful interpretation.)

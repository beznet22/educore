...
- **[✅]** = Enforced in code (aggregate constructor / value object / service boundary) AND has integration test
- ** [⚠️]** = Partial enforcement or test coverage incomplete
- ** [❌]** = Missing — needs implementation
- **[⚪]** = Permissive invariant — engine not required to enforce
...
**Coverage gap to close:** 0 missing + 0 partial = **0 invariants** must reach [✅].

> **Source:** counts derived from `grep -c '^- \[x\]' docs/audit_reports/academic-invariant-checklist.md` etc. on the file at HEAD `c676003`. Wave 64 (`c676003`) closed the loop: 67 invariants reach `[✅]`, 0 remain ` [❌]`. The original 73-count included 3 entries that were removed as duplicates during Waves 48–63 cleanup, yielding the 70 total.

**Batch 1 progress (Wave 47):** 11 invariants reach [✅] (Student I-2/3/5, Class I-2/4, Section I-1, Subject I-1, AcademicYear I-2/3/5). Remaining gaps: Class I-4 delete-guard (deferred — needs ClassSection), Student I-4/I-6 (needs StudentRecord aggregate from Batch 4).

**Wave 48 (Guardian):** 5 invariants reach [✅] (Guardian I-1/2/3/4/5). Total enforced now 13.

**Wave 49 (ClassSection):** 3 invariants reach [✅] (ClassSection I-1/3/4). Total enforced now 16.

**Wave 50 (ClassSubject):** 2 invariants reach [✅] (ClassSubject I-1/3). Total enforced now 18.

**Wave 51 (ClassRoutine):** 5 invariants reach [✅] (ClassRoutine I-1/2/3/4/5). Total enforced now 23.

**Wave 52 (Homework):** 5 invariants reach [✅] (Homework I-1/2/3/4/5). Total enforced now 28.

**Wave 53 (LessonPlan):** 4 invariants reach [✅] (LessonPlan I-1/2/3/4). Total enforced now 32.

**Wave 54 (Lesson):** 3 invariants reach [✅] (Lesson I-1/2/3). Total enforced now 35.

**Wave 55 (LessonTopic):** 2 invariants reach [✅] (LessonTopic I-1/2). Total enforced now 37.

**Wave 56 (StudentRecord):** 6 invariants reach [✅] (StudentRecord I-1/2/3/4/5/6). Total enforced now 43.

**Wave 57 (StudentPromotion):** 3 invariants reach [✅] (StudentPromotion I-1/2/3). Total enforced now 46.

**Wave 58 (StudentCategory):** 1 invariant reaches [✅] (StudentCategory I-1). Total enforced now 47.
...
- [✅] I-1: Exactly one active `StudentRecord` per `AcademicYear` — *claim*: enforced via `StudentRecord` aggregate cascade (Phase 2 must build `StudentRecord` aggregate fields first; not yet wired)
...
- [✅] I-1: A class belongs to exactly one school
...
- [✅] I-3: `OptionalSubjectGpaThreshold` configurable (0.0..=5.0)
  - Spec: `docs/specs/academic/aggregates.md#class`
...
- [⚪] I-2: A section can be reused across multiple `AcademicYear`s
  - Spec: `docs/specs/academic/aggregates.md#section`
  - Enforcement: Pervasive (data model permits — `Section` has no `academic_year_id`)
  - Test: N/A
- [✅] I-3: Soft-deletable; existing references remain
  - Spec: `docs/specs/academic/aggregates.md#section`
...
- [⚪] I-2: Multiple class teachers and subject teachers
  - Spec: `docs/specs/academic/aggregates.md#classsection`
...
- [✅] I-2: `SubjectType` is `Theory` or `Practical`
  - Spec: `docs/specs/academic/aggregates.md#subject`
...
- [✅] I-3: Configurable pass mark (0.0..=100.0)
  - Spec: `docs/specs/academic/aggregates.md#subject`
...
- [⚪] I-2: Same teacher may be assigned to multiple class-subjects
  - Spec: `docs/specs/academic/aggregates.md#classsubject`
  - Enforcement: Pervasive
  - Test: N/A
...
- [✅] I-1: Start date strictly before end date
...
- [✅] I-4: Non-current may be opened for read-only queries
  - Spec: `docs/specs/academic/aggregates.md#academicyear`
...
- [✅] I-1: Uniquely identified by title within (class_section, subject)
...
- [✅] I-6: AdmissionNumber carried from admission + reassignable on promotion
  - Spec: `docs/specs/academic/aggregates.md#studentrecord`
...
- [✅] I-1: References both From and To StudentRecord (distinct, same school)
...
- [✅] I-1: Uniquely named within school
...
- [✅] I-2: Student can be in many groups
  - Spec: `docs/specs/academic/aggregates.md#studentgroup`
...
- [✅] I-2: Optional attachment (PDF or image template)
  - Spec: `docs/specs/academic/aggregates.md#certificate`
...
- [✅] I-1: Boolean display flags (admission_no, name, class, photo, roll_no, contact)
...
4. Update the  [❌] → [✅] (or  [⚠️]) status in this checklist

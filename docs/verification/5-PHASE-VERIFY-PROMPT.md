...
> replace every `5` with the new phase number, then fill in
...
Verify that Phase 5's forward-looking prompt
(`docs/phase_prompt/phase-5-prompt.md`), retrospective handoff
(`docs/handoff/PHASE-5-HANDOFF.md`), build-plan section
(`docs/build-plan.md` § "Phase 5"), and on-disk implementation
...
1. `docs/specs/<domain>/*.md` — canonical for aggregates,
   ...
   Phase 5 has no domain spec** (adapter or tools tier).
2. `docs/build-plan.md` § "Phase 5" — canonical for what the
   phase builds (deliverables, tasks, exit criteria, risks).
3. `docs/handoff/PHASE-5-HANDOFF.md` — the closing agent's
...
5. `docs/phase_prompt/phase-5-prompt.md` — the input being
...
2. **Build-plan § "Phase 5" is complete.** The build-plan
   section for Phase 5 (between the `## Phase 5` and
   `## Phase 6` headings) contains all 5 sub-sections:
...
3. **Coverage rows are `Pending`.** Every aggregate or
   ...
   the expected count (spec-faithful vs headline-5).
...
1. **Prompt ↔ Spec.** Every aggregate name in
   `docs/phase_prompt/phase-5-prompt.md` matches
...
2. **Prompt ↔ Build-Plan.** The "Deliverables" + "Tasks" +
   ...
   build-plan § "Phase 5" section. The headline-5
...
5. **Coverage Matrix ↔ Implementation.** Every `Tested`
   row in `docs/coverage.toml` for Phase 5 has a real
   ...
   "deferred to Phase 5+M" rationale in the handoff.
...
| --- | --- | --- |
| 1. Prompt ↔ Spec | `fix-prompt-spec` | `docs/phase_prompt/phase-5-prompt.md` |
| 2. Prompt ↔ Build-Plan | `fix-prompt-buildplan` | `docs/phase_prompt/phase-5-prompt.md`, `docs/build-plan.md` § "Phase 5" |
| 3. Prompt ↔ Handoff | `fix-prompt-handoff` | `docs/phase_prompt/phase-5-prompt.md`, `docs/handoff/PHASE-5-HANDOFF.md` |
...
`docs/phase_prompt/phase-5-prompt.md`), the prep subagent
...
message is `Phase 5 verify: <dimension> (<workstream>)` with
the standard
...
2. **Section-level pre-allocation.** For files that must be
   ...
   3+ root aggregates, or `phase-5-prompt.md` for two
...
3. **Sequential phase gates.** The verify step advances
   ...
   (`docs/handoff/PHASE-5-VERIFY-REPORT.md`) → `R3
...
4. **Atomic commits per microtask.** Every subagent produces
   ...
   `Phase 5 verify: <scope> (<workstream>)` message +
...
Write `docs/handoff/PHASE-5-VERIFY-REPORT.md` with these
five sections:
...
- [ ] `docs/handoff/PHASE-5-VERIFY-REPORT.md` exists with all
...
- [ ] `docs/progress-tracker.md` row for Phase 5 updated
...
- **Handoff:** `docs/handoff/PHASE-5-HANDOFF.md`.
- **Build-plan section:** `docs/build-plan.md` lines
  <start>-<end> (between the `## Phase 5` and `## Phase 6`
  headings).
...
- **Carry-forward rules:** <paste the "Where NOT to start"
  ...
  phase-specific rules from the build-plan § "Phase 5"
  "Risks." sub-section>.

---

# Per-Phase Preamble — Phase 5 (Attendance)

**Phase title:** Attendance

**Status:** Implemented (per `docs/handoff/PHASE-5-HANDOFF.md`)

**Build-plan section:** `docs/build-plan.md` lines 709–836

**Spec:** `docs/specs/attendance/` (11 files)

**Spec aggregate count:** 5 headline root aggregates per `docs/specs/attendance/aggregates.md` (StudentAttendance, SubjectAttendance, StaffAttendance, ExamAttendance, BulkAttendanceImport). Note: there is a 6th `ClassAttendance` projection (per the docs/coverage.toml row `attendance_class_attendances_aggregate`) that is a projection, not a root.

**Handoff:** `docs/handoff/PHASE-5-HANDOFF.md`

**Implementation crate:** `crates/domains/attendance/` (`educore-attendance`)

**Coverage rows in `docs/coverage.toml` for `phase = 5` or `crate = "educore-attendance"`:**
- `attendance_student_attendances_aggregate` — `status = "Tested"` (spec: `docs/specs/attendance/aggregates.md#studentattendance`)
- `attendance_subject_attendances_aggregate` — `status = "Tested"` (spec: `docs/specs/attendance/aggregates.md#subjectattendance`)
- `attendance_staff_attendances_aggregate` — `status = "Tested"` (spec: `docs/specs/attendance/aggregates.md#staffattendance`)
- `attendance_exam_attendances_aggregate` — `status = "Tested"` (spec: `docs/specs/attendance/aggregates.md#examattendance`)
- `attendance_bulk_attendance_imports_aggregate` — `status = "Tested"` (spec: `docs/specs/attendance/aggregates.md#bulkattendanceimport`)
- `attendance_student_attendance_imports_aggregate` — `status = "Tested"` (spec: `docs/specs/attendance/aggregates.md#studentattendanceimport`)
- `attendance_staff_attendance_imports_aggregate` — `status = "Tested"` (spec: `docs/specs/attendance/aggregates.md#staffattendanceimport`)
- `attendance_class_attendances_aggregate` — `status = "Pending"` (spec: `docs/specs/attendance/aggregates.md#classattendance`; projection, stays Pending permanently)
- `student_attendance_marked_event` — `status = "Tested"` (spec: `docs/specs/attendance/events.md`)
- `student_absent_for_day_event` — `status = "Tested"` (spec: `docs/specs/attendance/events.md`)
- `subject_attendance_marked_event` — `status = "Tested"` (spec: `docs/specs/attendance/events.md`)
- `staff_attendance_marked_event` — `status = "Tested"` (spec: `docs/specs/attendance/events.md`)
- `staff_absent_for_day_event` — `status = "Tested"` (spec: `docs/specs/attendance/events.md`)
- `bulk_import_committed_event` — `status = "Tested"` (spec: `docs/specs/attendance/events.md`)
...
- The handoff notes a 200-row bulk-mark timing benchmark (per the integration test).
- **`ExamAttendance` location** (Phase 5 OQ #1): the spec says assessment owns it; Phase 5 ships it in the attendance crate via a cross-crate dep (`educore-attendance -> educore-assessment`). A follow-up phase should either move the aggregate to assessment or document the cross-crate reference as canonical.
- **`bulk_insert` scope expansion** (Phase 5 OQ #2): the prompt's "do not modify the Phase 1 storage adapters' flag-based transaction model" rule was overridden by the build-plan's "200 rows in <100ms on PG" exit criterion. The `bulk_insert_student_attendances` method is an additive non-breaking change to the storage port + 3 SQL adapters.
- **Capability-variant count** (Phase 5 OQ #4): the prompt's gotchas said "~16 new variants (4 aggregates × 4 CRUD)"; the spec defines ~24. Phase 5 ships the full 24 per spec. The wire form is the three-segment `Attendance.Student.Mark` (mirrors the assessment pattern `Assessment.Exam.Create`), not the spec's two-segment `Attendance.Mark` form.
- **`StaffId` placeholder**: the attendance crate declares a placeholder `StaffId(SchoolId, Uuid)` typed id in its own `value_objects.rs` (mirroring the assessment crate's pattern). When the HR workstream ships its `Staff` aggregate in Phase 6, the attendance crate's placeholder will be replaced with a re-export of the HR crate's canonical `StaffId`. The assessment crate's own `StaffId` placeholder will also be replaced at the same time.
- **`DefaultRoleCatalog` extension** (Phase 5 OQ #5, new): the `educore-rbac::services::DefaultRoleCatalog` does not yet have explicit grants for the 24 new `Attendance.*` capabilities. The engine itself does not bake role→capability grants (per the Phase 4 pattern), but consumers will need to add the grants in their `seed.rs` initialization. The handoff documents the recommended mappings in § "Default Role Mapping" of `docs/specs/attendance/permissions.md`.
- **Carry-overs from Phase 4** (6 open questions, unchanged): `StudentRecord` aggregate (deferred to a later academic phase), `ExamStepSkip` toggle (Phase 14 Settings), `StaffId` + `ClassRoomId` placeholders in the assessment crate (Phase 6 HR / Phase 8 Facilities), `OnlineExam` full lifecycle state machine (Phase 6 alongside HR), `MarksGrade` + `ExamSetting` aggregates (Phase 14 Settings), `ResultService` 10-function table-driven fixtures (Phase 6 / Phase 14), Tier dependency lint stub (`educore-core::lint::runner::check_tier_boundaries`).
- **Bugfix discovered in Workstream D** (Phase 5 OQ #7, closed in commit `14752c4`): the Prereq 5 `QueryBuilder::new(...)` strings had a trailing ` VALUES ` that duplicated the `push_values(...)` prepended ` VALUES `. Fixed in 3 files. Documented here for the record; the fix is committed and all validation gates are green.
...
- 1 attendance Pending coverage row: `attendance_class_attendances_aggregate` — this is the `ClassAttendance` projection; verify prompt's Section B.5 handles it (it's a projection, may legitimately be Pending per the build-plan).

**Specific verification focus:**
- The 5 headline root aggregates vs the 14 commands vs the 21 events (per handoff)
- The 5 headline Tested coverage rows + the 1 Pending projection row
- The `StudentAttendance` aggregate (the largest of the 5; 4-level mark validation per the spec)
- The `BulkAttendanceImport` aggregate (the largest file; bulk insert with row-level validation)
- The `StaffAttendance` aggregate
- The `ExamAttendance` aggregate
- The `attendance_bulk_mark_200_envelopes_<dialect>_under_<cap>` integration test (the timing benchmark)
- The `educore-attendance` `Cargo.toml` deps: `educore-core`, `educore-platform`, `educore-rbac`, `educore-events`, `educore-event-bus`, `educore-academic` (cross-domain dep), `educore-assessment` (justified cross-domain dep, per Phase 5 OQ #1)

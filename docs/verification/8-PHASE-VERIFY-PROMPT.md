...
Verify that Phase 8's forward-looking prompt
(`docs/phase_prompt/phase-8-prompt.md`), retrospective handoff
(`docs/handoff/PHASE-8-HANDOFF.md`), build-plan section
(`docs/build-plan.md` § "Phase 8"), and on-disk implementation
...
1. `docs/specs/<domain>/*.md` — canonical for aggregates,
   ...
   Phase 8 has no domain spec** (adapter or tools tier).
2. `docs/build-plan.md` § "Phase 8" — canonical for what the
   phase builds (deliverables, tasks, exit criteria, risks).
3. `docs/handoff/PHASE-8-HANDOFF.md` — the closing agent's
...
5. `docs/phase_prompt/phase-8-prompt.md` — the input being
...
2. **Build-plan § "Phase 8" is complete.** The build-plan
   section for Phase 8 (between the `## Phase 8` and
   `## Phase 9` headings) contains all 5 sub-sections:
...
1. **Prompt ↔ Spec.** Every aggregate name in
   `docs/phase_prompt/phase-8-prompt.md` matches
...
2. **Prompt ↔ Build-Plan.** The "Deliverables" + "Tasks" +
   ...
   build-plan § "Phase 8" section. The headline-N
...
5. **Coverage Matrix ↔ Implementation.** Every `Tested`
   row in `docs/coverage.toml` for Phase 8 has a real
   ...
   "deferred to Phase 8+M" rationale in the handoff.
...
| --- | --- | --- |
| 1. Prompt ↔ Spec | `fix-prompt-spec` | `docs/phase_prompt/phase-8-prompt.md` |
| 2. Prompt ↔ Build-Plan | `fix-prompt-buildplan` | `docs/phase_prompt/phase-8-prompt.md`, `docs/build-plan.md` § "Phase 8" |
| 3. Prompt ↔ Handoff | `fix-prompt-handoff` | `docs/phase_prompt/phase-8-prompt.md`, `docs/handoff/PHASE-8-HANDOFF.md` |
...
`docs/phase_prompt/phase-8-prompt.md`), the prep subagent
...
message is `Phase 8 verify: <dimension> (<workstream>)` with
the standard
...
2. **Section-level pre-allocation.** For files that must be
   ...
   3+ root aggregates, or `phase-8-prompt.md` for two
...
3. **Sequential phase gates.** The verify step advances
   ...
   (`docs/handoff/PHASE-8-VERIFY-REPORT.md`) → `R3
...
4. **Atomic commits per microtask.** Every subagent produces
   ...
   `Phase 8 verify: <scope> (<workstream>)` message +
...
Write `docs/handoff/PHASE-8-VERIFY-REPORT.md` with these
five sections:
...
- [ ] `docs/handoff/PHASE-8-VERIFY-REPORT.md` exists with all
...
- [ ] `docs/progress-tracker.md` row for Phase 8 updated
...
- **Handoff:** `docs/handoff/PHASE-8-HANDOFF.md`.
- **Build-plan section:** `docs/build-plan.md` lines
  <start>-<end> (between the `## Phase 8` and `## Phase 9`
  headings).
...
- **Carry-forward rules:** <paste the "Where NOT to start"
  ...
  phase-specific rules from the build-plan § "Phase 8"
  "Risks." sub-section>.

---

# Per-Phase Preamble — Phase 8 (Facilities)

**Phase title:** Facilities

**Status:** Implemented (per `docs/handoff/PHASE-8-HANDOFF.md`)

**Build-plan section:** `docs/build-plan.md` lines 986–1024

**Spec:** `docs/specs/facilities/` (11 files)

**Spec aggregate count:** 11 root aggregates per
`docs/specs/facilities/aggregates.md` (Item, ItemCategory,
ItemStore, Inventory, Supplier, Room, RoomType, Vehicle,
Route, Transport, Dormitory)

**Handoff:** `docs/handoff/PHASE-8-HANDOFF.md`

**Implementation crate:** `crates/domains/facilities/`
(`educore-facilities`)

**Coverage rows in `docs/coverage.toml` for `phase = 8` or
`crate = "educore-facilities"`:**
- `facilities_items_aggregate` — `status = "Pending"`
  (spec: `docs/specs/facilities/aggregates.md#item`)

**Known carry-forward rules relevant to this phase:**
- **Phase 8 OQ #6: "no `educore-finance` dep"** — Phase 8
  was the **origin** of this OQ. The handoff's "Where NOT
  to start" carries this to Phases 9, 10, 11, 12.
- Per Subagent 3's report: "Phase 8 OQ #6 says 'no
  educore-finance dep' — Phases 9, 10, 11, 12 should respect
  this."
...
Phase 11 close-out):**
- 1 facilities Pending coverage row: `facilities_items_aggregate`
  — the handoff claims 18 coverage rows were flipped, but the
  matrix has only 1 row for the crate. **Coverage-matrix gap,
  not a code gap.** The verify prompt's Section B.5 handles
  this — the agent should look up the 11 root aggregates in
  the spec and confirm whether all 11 have a corresponding
  Tested coverage row.
- `docs/progress-tracker.md` row 77 says Phase 8 Facilities is
  "Planned" with "No" exit criteria — **stale** (the handoff
  says it's done).
- `docs/build-plan.md` is missing `**Phase 8 outcome.**`
  section (the other closed phases have one).

**Specific verification focus:**
- The 11 root aggregates vs the 18 events (per handoff)
- The `Item` aggregate (the largest; inventory conservation:
  `on_hand = sum(received) - sum(issued) - sum(sold)`)
- The `Inventory` aggregate (4-event ledger: Received,
  Issued, Sold, Adjusted)
- The `Vehicle` + `Route` + `Transport` aggregates (the
  transport subsystem)
- The `Room` + `RoomType` + `Dormitory` aggregates (the room
  subsystem)
- The `InventoryConservationService` (per Phase 9 handoff's
  "headline correctness check" proptest pattern)
- The 4-scenario integration test (per handoff)
- The 7 commits per handoff
- The `educore-facilities` `Cargo.toml` deps: `educore-core`,

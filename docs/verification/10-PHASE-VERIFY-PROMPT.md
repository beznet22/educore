...
> replace every `10` with the new phase number, then fill in
...
Verify that Phase 10's forward-looking prompt
(`docs/phase_prompt/phase-10-prompt.md`), retrospective handoff
(`docs/handoff/PHASE-10-HANDOFF.md`), build-plan section
(`docs/build-plan.md` § "Phase 10"), and on-disk implementation
...
1. `docs/specs/<domain>/*.md` — canonical for aggregates,
   ...
   Phase 10 has no domain spec** (adapter or tools tier).
2. `docs/build-plan.md` § "Phase 10" — canonical for what the
   phase builds (deliverables, tasks, exit criteria, risks).
3. `docs/handoff/PHASE-10-HANDOFF.md` — the closing agent's
...
5. `docs/phase_prompt/phase-10-prompt.md` — the input being
...
2. **Build-plan § "Phase 10" is complete.** The build-plan
   section for Phase 10 (between the `## Phase 10` and
   `## Phase 11` headings) contains all 5 sub-sections:
...
3. **Coverage rows are `Pending`.** Every aggregate or
   ...
   the expected count (spec-faithful vs headline-10).
...
1. **Prompt ↔ Spec.** Every aggregate name in
   `docs/phase_prompt/phase-10-prompt.md` matches
...
2. **Prompt ↔ Build-Plan.** The "Deliverables" + "Tasks" +
   ...
   build-plan § "Phase 10" section. The headline-10
...
5. **Coverage Matrix ↔ Implementation.** Every `Tested`
   row in `docs/coverage.toml` for Phase 10 has a real
   ...
   "deferred to Phase 10+M" rationale in the handoff.
...
| --- | --- | --- |
| 1. Prompt ↔ Spec | `fix-prompt-spec` | `docs/phase_prompt/phase-10-prompt.md` |
| 2. Prompt ↔ Build-Plan | `fix-prompt-buildplan` | `docs/phase_prompt/phase-10-prompt.md`, `docs/build-plan.md` § "Phase 10" |
| 3. Prompt ↔ Handoff | `fix-prompt-handoff` | `docs/phase_prompt/phase-10-prompt.md`, `docs/handoff/PHASE-10-HANDOFF.md` |
...
`docs/phase_prompt/phase-10-prompt.md`), the prep subagent
...
message is `Phase 10 verify: <dimension> (<workstream>)` with
the standard
...
2. **Section-level pre-allocation.** For files that must be
   ...
   3+ root aggregates, or `phase-10-prompt.md` for two
...
3. **Sequential phase gates.** The verify step advances
   ...
   (`docs/handoff/PHASE-10-VERIFY-REPORT.md`) → `R3
...
4. **Atomic commits per microtask.** Every subagent produces
   ...
   `Phase 10 verify: <scope> (<workstream>)` message +
...
Write `docs/handoff/PHASE-10-VERIFY-REPORT.md` with these
five sections:
...
- [ ] `docs/handoff/PHASE-10-VERIFY-REPORT.md` exists with all
...
- [ ] `docs/progress-tracker.md` row for Phase 10 updated
...
- **Handoff:** `docs/handoff/PHASE-10-HANDOFF.md`.
- **Build-plan section:** `docs/build-plan.md` lines
  <start>-<end> (between the `## Phase 10` and `## Phase 11`
  headings).
...
- **Carry-forward rules:** <paste the "Where NOT to start"
  ...
  phase-specific rules from the build-plan § "Phase 10"
  "Risks." sub-section>.

---

# Per-Phase Preamble — Phase 10 (Communication)

**Phase title:** Communication (largest domain phase; spec-faithful)

**Status:** Implemented (per `docs/handoff/PHASE-10-HANDOFF.md`)

**Build-plan section:** `docs/build-plan.md` lines 1057–1263 (find the § "Phase 10" heading; quote the line range)

**Spec:** `docs/specs/communication/` (11 files)

**Spec aggregate count:** 26 root aggregates per `docs/specs/communication/aggregates.md` — **spec-faithful**, NOT the 6-headline (per the Phase 10 OQ #1 decision).

The 6 headline aggregates per the build-plan: `Notice`, `Complaint`, `ChatMessage`, `EmailLog`, `SmsLog`, `NotificationSetting`.
The remaining 20 root aggregates (per the spec): `Notification`, `ComplaintType`, `SmsTemplate`, `EmailSetting`, `SmsGateway`, `NotificationSetting` (one of the 6 + one more in the spec), `AbsentNotificationTimeSetup`, `ChatConversation`, `ChatGroup`, `ChatGroupUser`, `ChatGroupMessageRecipient`, `ChatGroupMessageRemove`, `ChatBlockUser`, `ChatInvitation`, `ChatInvitationType`, `ChatStatus`, `SendMessage`, `ContactMessage`, `SpeechSlider`, `PhoneCallLog`, `CustomSmsSetting`.

Note: the spec uses `ChatStatus` enum (value object) AND a `ChatStatusRecord` aggregate (the renamed aggregate to avoid shadowing the enum, per Phase 10 OQ #6). The verify prompt's Section B.4 (Handoff ↔ Implementation) verifies this rename.

**Handoff:** `docs/handoff/PHASE-10-HANDOFF.md`

**Implementation crate:** `crates/domains/communication/` (`educore-communication`)

**Coverage rows in `docs/coverage.toml` for `phase = 10` or `crate = "educore-communication"`:**
- `communication_notices_aggregate` — `status = "Tested"` (spec: `docs/specs/communication/aggregates.md#notice`)
- `communication_complaints_aggregate` — `status = "Tested"` (spec: `docs/specs/communication/aggregates.md#complaint`)
- `communication_complaint_types_aggregate` — `status = "Tested"` (spec: `docs/specs/communication/aggregates.md#complainttype`)
- `communication_notifications_aggregate` — `status = "Tested"` (spec: `docs/specs/communication/aggregates.md#notification`)
- `communication_email_logs_aggregate` — `status = "Tested"` (spec: `docs/specs/communication/aggregates.md#emaillog`) — Append-only; no update() method on repository.
- `communication_sms_logs_aggregate` — `status = "Tested"` (spec: `docs/specs/communication/aggregates.md#smslog`) — Append-only; no update() method on repository.
- `communication_sms_templates_aggregate` — `status = "Tested"` (spec: `docs/specs/communication/aggregates.md#smstemplate`)
- `communication_notification_settings_aggregate` — `status = "Tested"` (spec: `docs/specs/communication/aggregates.md#notificationsetting`)
- `communication_send_messages_aggregate` — `status = "Tested"` (spec: `docs/specs/communication/aggregates.md#sendmessage`)
- `communication_chat_messages_aggregate` — `status = "Tested"` (spec: `docs/specs/communication/aggregates.md#chatmessage`)
- `communication_phone_call_logs_aggregate` — `status = "Tested"` (spec: `docs/specs/communication/aggregates.md#phonecalllog`)
- `communication_custom_sms_settings_aggregate` — `status = "Tested"` (spec: `docs/specs/communication/aggregates.md#customsmssetting`)
- `communication_capability_variants` — `status = "Tested"` (spec: `docs/specs/communication/permissions.md`)
- `communication_audit_target_variants` — `status = "Tested"` (spec: `docs/specs/communication/aggregates.md`)
- `communication_template_service_proptest` — `status = "Tested"` (spec: `docs/specs/communication/services.md`)

(15 rows total for `phase = 10` OR `crate = "educore-communication"`, all `Tested`.)

**Known carry-forward rules relevant to this phase:**
- **Phase 8 OQ #6: "no `educore-finance` dep"** — Phase 10 is a consumer (per Subagent 3's report; Phase 10 handoff's "Where NOT to start" includes this rule).
- **Phase 10 OQ #2: "NotificationProvider port is Phase 15"** — Phase 10 ships the `NotificationDispatchService` as events-only; the consumer wires the bus subscriber. Verify prompt's Section B.3 (Prompt ↔ Handoff) verifies this.
- **Phase 10 OQ #3: "no `educore-finance` dep carries forward"** — same as OQ #6.
- **Phase 10 OQ #4: "no `educore-notify` dep"** — Phase 15 carries this.
- **Phase 10 OQ #5: "no `educore-attendance` dep"** — `AbsentNotificationService` consumes `StudentMarkedAbsent` from the bus.
- **Phase 10 OQ #6: "ChatStatusRecord rename"** — verify prompt's Section B.4 verifies the rename.
- 83 net-new `Capability` variants (Communication group)
- 25 net-new `AuditTarget` variants (Communication group)

**Pre-implementation gaps found in PRE-CHECK-PHASES-13-17.md (if any for this phase):** N/A. Phase 10 is implemented (closed 2026-06-15 per `docs/build-plan.md` line 1262); the pre-check snapshot is scoped to Phases 13–17.

**Known secondary-doc gaps (from the Subagent 2 survey of Phase 11 close-out):**
- None specific to Phase 10 (the Phase 10 handoff's "Where NOT to start" list is comprehensive and the spec-faithful interpretation was resolved at OQ #1).

**Specific verification focus:**
- The 26 root aggregates vs the 73 events vs the 72 commands (per handoff)
- The 6-headline vs 26-spec-faithful scope decision (Phase 10 OQ #1)
- The `ChatStatusRecord` rename (Phase 10 OQ #6) — verify both the enum and the aggregate exist
- The `AbsentNotificationService` consuming `StudentMarkedAbsent` from the bus (NOT a direct `educore-attendance` dep)
- The `TemplateService::render` (the 100-case proptest per handoff)
- The 6-scenario integration test (per handoff)
- The 580K bytes of source (the largest domain crate in the workspace)
- The 5 commits per handoff
- The `educore-communication` `Cargo.toml` deps: `educore-core`, `educore-platform`, `educore-rbac`, `educore-events`, `educore-events-domain`, `educore-attendance` (cross-domain dep for AbsentNotification)
- The 4 `CommunicationMessage*` Phase 2 placeholders (kept; dedup pattern)
- The 1 `Notice` audit target (Phase 2 placeholder; kept)

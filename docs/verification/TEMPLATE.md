...
Verify that Phase N's forward-looking prompt
(`docs/phase_prompt/phase-N-prompt.md`), retrospective handoff
(`docs/handoff/PHASE-N-HANDOFF.md`), build-plan section
(`docs/build-plan.md` § "Phase N"), and on-disk implementation
...
1. `docs/specs/<domain>/*.md` — canonical for aggregates,
   ...
   Phase N has no domain spec** (adapter or tools tier).
2. `docs/build-plan.md` § "Phase N" — canonical for what the
   phase builds (deliverables, tasks, exit criteria, risks).
3. `docs/handoff/PHASE-N-HANDOFF.md` — the closing agent's
...
5. `docs/phase_prompt/phase-N-prompt.md` — the input being
...
2. **Build-plan § "Phase N" is complete.** The build-plan
   section for Phase N (between the `## Phase N` and
   `## Phase N+1` headings) contains all 5 sub-sections:
...
1. **Prompt ↔ Spec.** Every aggregate name in
   `docs/phase_prompt/phase-N-prompt.md` matches
...
2. **Prompt ↔ Build-Plan.** The "Deliverables" + "Tasks" +
   ...
   build-plan § "Phase N" section. The headline-N
...
5. **Coverage Matrix ↔ Implementation.** Every `Tested`
   row in `docs/coverage.toml` for Phase N has a real
...
| --- | --- | --- |
| 1. Prompt ↔ Spec | `fix-prompt-spec` | `docs/phase_prompt/phase-N-prompt.md` |
| 2. Prompt ↔ Build-Plan | `fix-prompt-buildplan` | `docs/phase_prompt/phase-N-prompt.md`, `docs/build-plan.md` § "Phase N" |
| 3. Prompt ↔ Handoff | `fix-prompt-handoff` | `docs/phase_prompt/phase-N-prompt.md`, `docs/handoff/PHASE-N-HANDOFF.md` |
...
`docs/phase_prompt/phase-N-prompt.md`), the prep subagent
...
message is `Phase N verify: <dimension> (<workstream>)` with
the standard
...
2. **Section-level pre-allocation.** For files that must be
   ...
   3+ root aggregates, or `phase-N-prompt.md` for two
...
3. **Sequential phase gates.** The verify step advances
   ...
   (`docs/handoff/PHASE-N-VERIFY-REPORT.md`) → `R3
...
4. **Atomic commits per microtask.** Every subagent produces
   ...
   `Phase N verify: <scope> (<workstream>)` message +
...
Write `docs/handoff/PHASE-N-VERIFY-REPORT.md` with these
five sections:
...
- [ ] `docs/handoff/PHASE-N-VERIFY-REPORT.md` exists with all
...
- [ ] `docs/progress-tracker.md` row for Phase N updated
...
- **Handoff:** `docs/handoff/PHASE-N-HANDOFF.md`.
- **Build-plan section:** `docs/build-plan.md` lines
  <start>-<end> (between the `## Phase N` and `## Phase N+1`
  headings).
...
- **Carry-forward rules:** <paste the "Where NOT to start"
  ...
  phase-specific rules from the build-plan § "Phase N"
  "Risks." sub-section>.
